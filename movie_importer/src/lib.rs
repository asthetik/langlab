...
pub fn read_txt_to_json(file_path: &path::PathBuf) -> Result<PathBuf, Box<dyn Error>> {
    ...
    for line in txt.lines().map(str::trim).filter(|l| !l.is_empty()) {
        if let Some(no) = disc_number(line, &disc_regex) {
            disc_no = no;
        } else if let Some(movie) = parse_move(disc_no, line, &movie_regex) {
            movies.push(movie);
        }
    }
...
fn save_to_json(movies: Vec<Movie>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let path = FileDialog::new()
        ...
        .set_directory(r"/Users/asthetik/Downloads")
        .save_file()
